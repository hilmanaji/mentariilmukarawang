			...
			foreach ($data_file->result() as $row) :  ?>
				<div class="col-md-3 animate-box">
					<div class="fh5co-event">
						<div class="date text-center"><span><?= date('d', strtotime($row->created_at)) ?><br><?= date('M y', strtotime($row->created_at)) ?></span></div>
						<h3><a href="#"><?= $row->keterangan ?></a></h3>
						<p>uploaded by : <?= $row->username ?>, <?= $row->nama ?></p>
						<p><a href="<?= base_url() ?>Home/file/<?= $row->id_file ?>/<?= $row->downloaded ?>">Download</a></p>
					</div>
				</div>	
			<?php 
			endforeach;
